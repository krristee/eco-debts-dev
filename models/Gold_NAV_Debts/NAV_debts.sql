/*debts:
    ...
    *, rowno() AS temp_field
    ...
    RESIDENT tmp_status_log;
     ...
     // add dummy records to debtors for actual status even when no sales are present for speciffic date
    CONCATENATE (debts)
    LOAD
    	...
    	debts.cust_vend_registration_no,
        debts.customer_code,
        'Not Group' 															AS debts.consolidation_group, //historical status used in calculation for not group companies 
        debts.customer_status_log,
        DATE(date) 																AS debts.posting_date
	RESIDENT tmp_status_log
    WHERE NOT EXISTS(debts.customer_search, debts.company_id & '|' & debts.customer_code & '|' & DATE(date));
    
    DROP FIELD debts.temp_field;
    DROP TABLE tmp_status_log;*/
WITH debts AS (
SELECT 
	"company_id",
	"vendor_code",
...
SELECT
    ...
    null as "CloseDate",
    null as "posting_profile"
FROM {{ref ('tmp_status_log') }}                sl
LEFT JOIN ( SELECT DISTINCT "company_id",
                "cust_vend_registration_no",
                "customer_code",
                "posting_date",
                "customer_search"
            FROM {{ref ('tmp_debts') }})  d   on EQUAL_NULL(d."customer_search", CONCAT(sl."company_id",'|', sl."customer_code",'|',DATE(sl."posting_date"))) 
...
'NAV'                                               AS "debts.system",
CAST("CloseDate"   AS DATE)                         as "debts.CloseDate",
