...
SELECT
    ...
    to_varchar(v."accountnum")				        AS "vendor_code",
	...
	trim(v."organizationcode_ecs")                  AS "cust_vend_registration_no", 
    ...
    concat(IFNULL(nb."Com",''), '|', IFNULL(to_varchar(v."accountnum"),''), '|', DATE(cle."transdate")) 
                                                    AS "customer_search",
  --- left join 
  DATE(cle."transdate")					            AS "posting_date",
    TO_VARCHAR(cle."transtype")                     AS "document_type_code", 
...
FROM {{source("FOD", "VENDTRANS")}}				        cle
left join {{ref('FO_map_NAV_BC')}}                      nb          on nb.DATAREAD = upper(cle."dataareaid")
left join {{source("FOD", "VENDTABLE")}}			    v           on v."accountnum" = cle."accountnum" and upper(v."dataareaid") = upper(cle."dataareaid") 
left join {{source("FOD", "DIRPARTYTABLE")}}            dp          on v."party" = dp."recid" 
LEFT JOIN {{ref('FO_map_name_by_registration_no')}}     mrn         on EQUAL_NULL(trim(v."organizationcode_ecs"), mrn."registration_no")
...
left join {{ref('FO_tmp_dim_from_gl')}}                 dim         on EQUAL_NULL(cle."Id" , dim."Id" )
...
LEFT JOIN {{ref ('FO_map_cust_vend_name') }}            cvn         on EQUAL_NULL(concat(nb."Com", '|2|', TO_VARCHAR(cle."accountnum")) , cvn."id")
...
SELECT
 	nb."Com" 							            AS "company_id",
    to_varchar(cle."accountnum")				    AS "vendor_code",
    cast(cle."recid" as nvarchar)				    AS "entry_no",
    to_varchar(cle."voucher")				        AS "document_no",-- Kristina : buvo to_varchar(cle."invoice")
    to_varchar(cle."invoice") 	                    AS "external_document_no",  -- Kristina : buvo cle."documentnum"  
    to_varchar(cle."voucher")			            AS "transaction_no",
    ...
    concat(IFNULL(nb."Com",''), '|', IFNULL(to_varchar(cle."accountnum"),''), '|', DATE(cle."transdate")) 
                                                    AS "customer_search",
...
FROM {{source("FOD", "VENDSETTLEMENT")}}               setl
left join {{source("FOD", "VENDTRANS")}}               cle          on setl."offsetrecid" = cle."recid"
left join {{source("FOD", "VENDTABLE")}}			   v            on v."accountnum" = cle."accountnum" and upper(v."dataareaid") = upper(cle."dataareaid")
left join {{source("FOD", "DIRPARTYTABLE")}}           dp           on v."party" = dp."recid" 
LEFT JOIN {{ref('FO_map_name_by_registration_no')}}    mrn          on EQUAL_NULL(trim(v."organizationcode_ecs"), mrn."registration_no")
left join {{ref('FO_map_NAV_BC')}}                     nb           on nb.DATAREAD = upper(cle."dataareaid")
